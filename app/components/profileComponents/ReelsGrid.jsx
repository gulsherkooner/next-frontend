  ...
  return (
    <div className="p-2 sm:p-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 md:gap-6">
        {reels.map((reel) => (
          <div key={reel.id} className="overflow-hidden">
            <div className="relative aspect-[9/16] bg-gray-800 overflow-hidden group">
              {/* Use a placeholder image for the reel thumbnail */}
              <div 
                className="absolute inset-0 bg-cover bg-center" 
                style={{ 
                  backgroundImage: `url(https://source.unsplash.com/random/?${['ocean', 'matrix', 'whale'][reel.id % 3]})` 
                }}
              />
              
              {/* Play button overlay */}
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                <button variant="ghost" size="icon" className="bg-black/30 text-white rounded-full h-12 w-12 sm:h-16 sm:w-16">
              ...
              <div className="absolute bottom-2 left-2 text-white text-xs sm:text-sm flex items-center">
                ...
                <span className="ml-1">{reel.viewCount}</span>
              </div>
            </div>
