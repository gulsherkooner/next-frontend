  ...
  const renderMenuItems = (items) => {
    return items.map((item, index) => (
      <Link
        ...
        className="flex items-center px-3 py-2 text-[14px] hover:bg-gray-200 rounded-lg transition-colors duration-200"
      >
        <span className="mr-4 text-neutral-700 flex-shrink-0">{item.icon}</span>
        <span className="text-neutral-500 font-bold">{item.text}</span>
      </Link>
    ));
  };

  // Updated sidebar class with proper height calculations
  ...
  return (
    <>
      ...
      <aside
        ...
        style={{ height: "calc(100vh - 3.5rem)" }} // 3.5rem = 56px (header height)
      >
